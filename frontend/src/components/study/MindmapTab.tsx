    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-red-400">
        ...
        <button
          ...
          className="mt-2 px-4 py-2 text-sm bg-gray-700 hover:bg-gray-600 disabled:opacity-60 text-white rounded-lg transition-colors"
        >
          다시 시도
        </button>
      </div>
  ...
  if (status === 'not_generated') {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 text-gray-400">
        <Network size={40} className="text-gray-600" />
  ...
  if (status === 'generating') {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-400">
        ...
        <span className="text-xs text-gray-600">잠시 후 페이지를 새로고침해 주세요</span>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-red-400">
        ...
        <button
          onClick={() => generate('mindmap')}
          disabled={isGenerating}
          className="flex items-center gap-2 mt-2 px-4 py-2 text-sm bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white rounded-lg transition-colors"
        >
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <button
        onClick={() => generate('mindmap')}
        disabled={isGenerating}
        className="absolute bottom-4 right-4 z-10 flex items-center gap-1.5 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 disabled:opacity-60 text-gray-300 text-xs rounded-lg border border-gray-700 transition-colors"
        title="마인드맵 재생성"
      >
