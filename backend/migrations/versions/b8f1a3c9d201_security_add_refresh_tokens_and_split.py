...
def upgrade() -> None:
    ...
    # Create refresh_tokens table for token revocation (idempotent)
    ...
    # Add selector column to password_reset_tokens if not present
    existing_columns = [c["name"] for c in inspector.get_columns("password_reset_tokens")]
    if "selector" not in existing_columns:
        op.add_column(
    ...
    if "ix_password_reset_tokens_selector" not in existing_indexes:
        op.create_index(
