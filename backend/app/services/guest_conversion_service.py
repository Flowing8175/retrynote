    ...
    async def move_guest_files(
        ...
        for f in files:
            if not f.stored_path or not f.stored_path.startswith("guest/"):
                continue
            try:
                filename = f.stored_path.split("/")[-1]
                new_path = f"{user_id}/{filename}"
                data = await _storage.download_file(f.stored_path)
                await _storage.upload_file(new_path, data)
                await _storage.delete_file(f.stored_path)
                f.stored_path = new_path
            except Exception:
                logger.warning("Failed to move file %s to %s", f.stored_path, user_id)
