...
b. 각주·예시의 세부 수치
c. 개념적 의미 없는 열거 순서 — 특히 "N단계/N번째에 해당하는 것은?" 형식 금지. 단계 번호와 개념명의 매핑은 순서 암기에 불과하다. 개념 간 인과·차이를 묻는 방향으로 재구성:
   ❌ "프로세스 5상태 모델에서 3번째 상태는?"
   ✅ "CPU 할당 대기 중인 프로세스와 입출력 완료를 기다리는 프로세스의 상태 차이는?"
   예외: 자료가 해당 번호 자체를 강조하거나, 번호 혼동이 핵심 오개념임을 명시한 경우.
