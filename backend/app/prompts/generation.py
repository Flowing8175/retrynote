...
- 빈칸형(fill_blank) 문제는 핵심 개념어(명사/용어) 자체를 빈칸으로 만든다.
  - 빈칸이 답이 되어야지, 자료의 서술 표현("~라고 설명한다", "~로 정의된다")이 문장을 감싸면 안 된다.
  - ❌ 나쁜 예: "자료에서는 자원을 '______'라고 설명한다." (자료 표현 복사형)
  - ✅ 좋은 예: "사회복지에서 욕구를 충족시키기 위해 동원되는 수단을 ______이라 한다." (개념 이해형)
  - 한 문장에 빈칸은 1개만 사용한다.
...
거절 규칙:
- 입력이 교육적으로 가치 있는 퀴즈를 만들기에 부적합하다고 판단되면, questions를 빈 배열로 두고 다음 형식으로 응답한다:
  {"questions": [], "rejected": true, "rejection_reason": "사용자에게 보여줄 짧은 거절 이유 (한국어)"}
- 거절 기준: 의미 없는 문자열("aaa", "111", "dummy" 등), 주제 없음(자료도 없고 topic도 없음), 퀴즈로 만들 수 없는 비교육적 내용(개인 일기, 영수증, 쇼핑 목록 등)
- 일반적인 주제("수학", "역사", "과학")는 거절하지 않는다 — 교육적 가치가 있으면 허용한다
- 거절 이유는 구체적이고 친절하게 작성한다 (예: "퀴즈를 만들기 어려운 내용입니다. 학습하고 싶은 주제나 자료를 입력해 주세요.")
