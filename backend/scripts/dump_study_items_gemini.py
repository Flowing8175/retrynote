"""Dump full JSON outputs of study_items prompts against Gemini 3.1 flash-lite.

Parallels dump_study_items.py (GPT version) but uses google-genai SDK.

GEMINI_API_KEY: pass via environment. If not set, falls back to project-root .env.
Remote-only Doppler setups can inject the key via:
  GEMINI_API_KEY=$(ssh ... 'doppler secrets get GEMINI_API_KEY --plain') \\
    backend/.venv/bin/python3 backend/scripts/dump_study_items_gemini.py

Usage:
  GEMINI_API_KEY=... /home/oh/dev/retrynote/backend/.venv/bin/python3 \\
    /home/oh/dev/retrynote/backend/scripts/dump_study_items_gemini.py
"""
...
async def run_case(client: "genai.Client", c: Case) -> dict:
    ...
    try:
        ...
        return {
            "case": c,
            "dt": time.perf_counter() - t0,
            "data": {"__api_error__": True, "__err__": f"{type(e).__name__}: {e}"},
    ...
    return {
        ...
        "usage": {
            ...
            "completion": getattr(usage, "candidates_token_count", None) if usage else None,
