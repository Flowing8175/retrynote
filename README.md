...
주요 API 카테고리:
- `/auth/` - 인증 (회원가입, 로그인, 비밀번호 재설정, SSE용 단기 stream-token 발급 `POST /auth/stream-token`)
- `/files/` - 파일 업로드 및 관리 (`POST /files` accepts mutually exclusive sources: `file` upload, `manual_text`, `source_url`, or `topic` + `topic_depth`)
...
### File source types (`POST /files`)

`POST /files` accepts exactly one of the following input modes. The processing worker normalizes all of them into a `ParsedDocument`, so all downstream features (study, quiz, search) treat them identically.

| Form field | `source_type` stored | Worker behavior |
|---|---|---|
| `file` (multipart) | `upload` | Parses PDF/DOCX/PPTX/TXT/MD/PNG/JPG, OCR for images and scanned docs |
| `manual_text` | `manual_text` | Stores the supplied text as-is |
| `source_url` | `url` | Fetches the URL with SSRF protection, extracts body text via trafilatura |
| `topic` (+ optional `topic_depth`) | `topic` | Calls the LLM to generate a canonical learning document (~1.2k / 3.5k / 7k chars for `brief` / `standard` / `deep`), stores it as parsed text |

The `topic` mode exists so direct-topic learning ("주제 직접 입력하기") shares a **single source of truth** with the rest of the pipeline: the AI-generated document is parsed once, then summary / flashcards / mindmap / concept-notes generators all read from it instead of independently improvising from the topic string.
